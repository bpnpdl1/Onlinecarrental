...
$owner_brand_id = request('owner_brand_id');
...
if (empty($name) || empty($user_id) || empty($price) || empty($model_year) || empty($seat) || empty($vehicle_no) || empty($description) || empty($owner_brand_id) ) {
...
$category = find('owner_brand', $owner_brand_id);
if (!$category) {
...
$to_delete2 = "../../uploads/" . $vehicle['paper_work'];
unlink($to_delete2);

update('owner_vehicle', $id, compact('name', 'user_id', 'price', 'model_year', 'seat', 'vehicle_no','description', 'owner_brand_id', 'image', 'paper_work'));
